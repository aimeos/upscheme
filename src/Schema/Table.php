...
class Table
{
	...
	protected function copyColumn( \Doctrine\DBAL\Schema\Column $column, $name ) : void
	{
		...
		if( $this->table->hasColumn( $name ) ) {
			$this->table->modifyColumn( $this->qi( $name ), $options + ['platformOptions' => $custom] );
		} else {
			$types = $column->getType()->getTypeRegistry();
			$this->table->addColumn( $this->qi( $name ), $types->lookupName( $column->getType() ), $options + ['platformOptions' => $custom] );
		}
	}
